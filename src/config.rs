...
pub fn config_dir() -> PathBuf {
    ...
    dirs::config_dir().unwrap_or_else(|| PathBuf::from(".")).join("webhookr")
}
...
impl ProjectConfig {
    ...
    pub fn new(
        ...
        Self { id, name, path, branch, command, secret, verify_mode }
    }
...
impl Default for AppConfig {
    fn default() -> Self {
        ...
        Self { listen_addr: format!("0.0.0.0:{port}"), projects: Vec::new() }
    }
}
...
pub fn save_config(cfg: &AppConfig) -> Result<()> {
    ...
    fs::write(&path, text).with_context(|| format!("failed to write config at {}", path.display()))?;
    Ok(())
}
